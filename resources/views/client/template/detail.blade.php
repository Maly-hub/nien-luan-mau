            ...
            <div class="row">
                <div class="col-6 order-1">
                    <div class="image_selected">
                        <img src="{{ asset('hinhanhsanpham') }}/{{ $sanPham->sp_hinhanh }}" style="width:400px;height:400px">
                    </div>
                </div>
                <!-- Description -->
                <div class="col-6 order-2">
                    <div class="product_description">
                    <form action="{{ route('gio-hang.them-nhieu-san-pham', ['idProduct'=>$sanPham->sp_id]) }}" method="post">
                        @csrf
                            <div class="product_name" >Tên sản phẩm: {{ $sanPham->sp_ten }}</div></br>
                            <div class="product_price">Giá sản phẩm: {{ number_format($sanPham->sp_gia) }}</div></br>
                            <div class="product_number">Số lượng: <input type="number" name="soLuong" value="1" min="1" max="{{ $sanPham->sp_soluong }}"></div></br>
                            <div>
                                <button type="submit" class="btn btn-primary mt-3"><i class="fa fa-shopping-cart" aria-hidden="true"></i>  Thêm vào giỏ</button>
                                <button type="button" class="btn btn-danger mt-3 ml-3" >Mua ngay</button>
                            </div>
                            <div >
                                <div class="btn btn-success mt-5" >
                                     <a href="{{ route('trang-chu') }}" style="color:honeydew">
